...
(function($) {
  ...
  THEMEMASCOT.initialize = {
    ...
    TM_SwiperSlider: function() {
      setTimeout(function() {
        $('.tm-swiper-carousel-arrow-wrap .tm-swiper-arrow-prev').on('click', function() {
          $(this).parents('.e-parent').find('.tm-swiper-button-wrap .tm-swiper-button-prev').trigger('click')
        }),
        $('.tm-swiper-carousel-arrow-wrap .tm-swiper-arrow-next').on('click', function() {
      ...
      if( ! $body.hasClass("elementor-editor-active") ) {
        ...
        $window.on("scroll", function() {
          if( $header_top_sticky.length > 0 ){
            var prevScrollpos  = $window.scrollTop();
            var $header_height = $header.height();
            var $navbar_height = $header_top_sticky.height();
            $window.on( 'scroll', function() {
              var currentScrollPos = $window.scrollTop();
              if ($header_height+100 > currentScrollPos) {
